const Profit = {
  ...
  getEffectiveCrafts(cantidad, rrr, materialesPorItem) {
  ...
  calculateAll() {
    ...
    Prices.TIERS.forEach((tier, tierIdx) => {
      Prices.ENCHANTS.forEach((ench, enchIdx) => {
        ...
        const costoFoco = usarFoco ? Math.round(costoTotal * 0.1) : 0;
        ...
        if (cells.length >= 8) {
          ...
          cells[7].textContent = costoFoco ? costoFoco.toLocaleString() : '-';
        }
      });
    });
  }
